...
> **MEASURED 2026-09-04, and it corrects the sentence above.** "Written only for moving objects"
> is what stock UE 4.27 does; it is not what THIS title does, and the difference is large.
> Over 4500 gameplay frames (9.33e9 px, `[STRAYDLSS] MvStats=1`, facts §37):
>
> ```
...
> ```
>
> **94% of what the engine writes is STATIC geometry** — `r.BasePassOutputsVelocity=True`
> (§2.3.1) makes the base pass write velocity for whatever the per-primitive gates let through,
> moving or not. Genuinely moving pixels are ~0.084% of the frame.
>
> **Two consequences.** Our reconstruction carries 96% of the field, so its correctness is not
> one branch of the motion-vector story but essentially all of it — which is why the
> `ClipToPrevClip` transposition fix mattered as much as it did. And the coverage
> `r.BasePassOutputsVelocity` adds is overwhelmingly redundant with what the camera
> reconstruction produces anyway, so it is not the lever it looks like.
>
> **A hypothesis closed by the same run:** the camera branch emits an exact zero on
