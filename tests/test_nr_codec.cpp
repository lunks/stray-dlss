...
TEST_CASE("the hybrid knee never reaches 1.0 and keeps highlight gradation")
{
	// The exp shoulder this replaced hit flat white at ~3.474; the hybrid knee is an unclipped
	// Neutwo of the excess, so 3x and 10x display white still encode to DIFFERENT proxy values.
	CHECK(soft_clip_channel(3.0f) < 1.0f);
	CHECK(soft_clip_channel(3.0f) > 0.99f);
	CHECK(soft_clip_channel(10.0f) > soft_clip_channel(3.0f));
	CHECK(soft_clip_channel(100.0f) < 1.0f);
	// Never ABOVE 1.0, which is the invariant the sRGB encode's domain actually depends on.
	// (At 1e6 float32 rounds the asymptote onto 1.0 exactly; that is allowed, exceeding is not.)
	CHECK(soft_clip_channel(1.0e6f) <= 1.0f);
	CHECK(soft_clip_channel(3.4739f) < 1.0f);
	// One scalar on the peak keeps hue: a saturated blue stays blue rather than turning cyan.
	const Float3 blue = soft_clip(Float3{ 0.10f, 0.20f, 2.00f });
	CHECK(near(blue.x / blue.z, 0.05f, 1e-4f));
	CHECK(near(blue.y / blue.z, 0.10f, 1e-4f));
	CHECK(blue.z < 1.0f);
	// Below the knee the vector form is the identity, per channel, whatever the other channels.
	const Float3 mid = soft_clip(Float3{ 0.10f, 0.50f, 0.75f });
	CHECK(mid.x == 0.10f);
	CHECK(mid.y == 0.50f);
	CHECK(mid.z == 0.75f);
}

TEST_CASE("the hybrid knee is C1: the slope is 1.0 on both sides of the knee")
{
	// The exp shoulder this replaced jumped from slope 1.0 to 1/ln(2) at the knee (C0, not C1).
	// Neutwo'(0) == 1, so the hybrid is C1 there, which is what keeps a gradient that crosses
	// the knee from showing a crease in the proxy the network is handed.
	const float h = 1e-4f;
	...
	CHECK(near(slope_above, 1.0f, 1e-2f));
}
