...
// THE HYBRID KNEE (2026-09-05, after OptiScaler_DLSSNR's dlssnr.hlsl "HybridCurve"): IDENTITY
// below the knee, and above it an unclipped Neutwo x*rsqrt(x*x+1) of the EXCESS, scaled into
// [k, 1). C1 at the knee; reaches 1.0 only asymptotically, so no channel ever clips and the
// sRGB encode's saturate never fires. The exp shoulder it replaces hit flat white by ~3.5x
// display white, so everything brighter was a band the network could not tell apart.
float NrSoftClipChannel(float value)
{
	if (value <= kNrSoftClipKnee)
		return value;
	const float headroom = 1.0f - kNrSoftClipKnee;
	const float e = (value - kNrSoftClipKnee) / headroom;   // excess above the knee, [0, inf)
	return kNrSoftClipKnee + headroom * (e * rsqrt(e * e + 1.0f));
}

// ONE SCALAR ON THE PEAK CHANNEL, so the roll-off cannot move hue. Rolling channels
// independently turns a saturated blue at B=2 into cyan (OptiScaler measured it as a green
// cast over every blue thing in GTA V); scaling the whole triple keeps the ratios.
float3 NrSoftClip(float3 color)
{
	color = max(color, 0.0f);
	const float m = max(color.r, max(color.g, color.b));
	if (m <= kNrSoftClipKnee || m <= 1e-6f)
		return color;
	return color * (NrSoftClipChannel(m) / m);
}
