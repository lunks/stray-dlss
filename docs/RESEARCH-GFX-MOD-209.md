...
| `r.HairStrands.DeepShadow.SuperSampling` | 1 | engine default | Hair shadow supersampling; Stray uses **gFur PRO** (`GiM-GamesInMotion/gFurPro`, a shell-fur plugin — **it is open source**, contrary to what this table assumed), so this cvar's reach is plugin-dependent. **RESOLVED 2026-09-03: Stray's fur does NOT use UE4's hair-strands pipeline.** gFur draws shells through its own vertex factories (`FFurSkinVertexFactory` and seven permutations, `FFurStaticVertexFactory`, `FPhysicsFurStaticVertexFactory` — all present as strings in the shipped exe, alongside `/Plugin/gFur/Private/GFurFactory.ush`), which have nothing to do with `FHairStrandsVertexFactory`. So every `r.HairStrands.*` cvar is a no-op for the cat. HARD, from the exe's own string table. |
...
| `r.NormalDeformer` | 1 | **INERT — CORRECTED 2026-09-03.** The string `NormalDeformer` does not appear anywhere in `Stray-Win64-Shipping.exe`, in ASCII or UTF-16LE (HARD: exhaustive printable-run extraction of the 85 MB binary in both encodings; the same pass finds `r.BasePassOutputsVelocity`, so the method fires). A cvar's name is a string literal in the binary that registers it, so a name that is absent is a cvar this build does not implement, and setting it does nothing. The "likely a GFurPRO-plugin cvar" guess is also wrong on its own terms — Stray's fur plugin registers exactly one cvar, `r.SkeletalMeshLODRadiusScale` (`FurComponent.cpp:138`). |
