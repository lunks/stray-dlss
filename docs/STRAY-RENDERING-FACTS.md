...
## §37 Motion-vector density, measured (2026-09-04) — the reconstruction IS the field

First measurement of the split between UE 4.27's own sparse velocity and our
depth + `ClipToPrevClip` reconstruction, from `[STRAYDLSS] MvStats=1` over 4500 gameplay
frames (9.33e9 pixels), The Slums, walked with the recorded scenario:

```
engine-written 3.91%   reconstructed 96.09%
of engine-written: genuinely moving 2.152%  still 94.04%  UNORM-clamped 0.0129%
of reconstructed: w<=0 rejected (emitted ZERO) 0.0000%
far/sky (device_z==0) 36.11% of frame, 0.0000% of those rejected
non-finite 0
|mv| px: <0.25 99.75%  <1 0.16%  <4 0.03%  <32 0.02%  >=32 0.038%
```

**Three things this settles.**

1. **Our reconstruction carries 96% of the frame.** Its correctness is not one branch of the
   motion-vector story, it is essentially the whole of it — which is the retroactive explanation
   for why the `ClipToPrevClip` transposition fix mattered as much as it did, and why a scale or
   convention error here compounds through every temporal consumer at once.
2. **`r.BasePassOutputsVelocity=True` buys almost nothing on this title.** Of the 3.91% the
   engine writes, **94.04% is static geometry that merely happened to be written**; only 2.152%
   of it — about 0.084% of the frame — is genuinely moving. The setting widens coverage over
   stock UE4 exactly as documented, and the coverage it adds is overwhelmingly redundant with
   what the camera reconstruction would have produced anyway.
3. **The `w<=0` rejection is EMPTY, and that closes a hypothesis rather than opening one.**
   `mv-dense-reflections` predicted this could be a second class of pixel we lie to DLSS about —
   the camera branch emits an exact zero on `prev_clip.w <= 0`, and a zero tells DLSS "this pixel
   did not move", a confident false statement. It measures **0.0000%** over 9.33e9 pixels. There
   is no such class here. Do not build the fix.

`non-finite 0` and `UNORM-clamped 0.0129%` are the instrument's own health checks; the report
refuses to print percentages when its invariants fail, so a printed table is itself evidence.

## §38 The pool-name route resolves the full G-buffer by engine name (2026-09-04)

`[STRAYDLSS] PoolNames=2` on the live game, gameplay in The Slums:

```
POOL NAMES: scan of 8 sections - literals found 19/20, names referenced 18, lea sites 21,
  distinct call targets 21. VERDICT: ok.
best candidate 0x6ffff9a4c740 - reached from 5 DISTINCT enclosing functions (.pdata-derived,
  bar is 3) and 18 distinct name literals (bar 4), nearest call 26 bytes after the name load.
scan took 291 ms on the device-creation thread. It runs ONCE.

[pool] mode=observe hooked=1 calls=71203 faults=0 off=0
  ok=45587 notRegistered=0 rhiNull=0 outNull=0 nameBad=0
  assert: depth 4144/0/0  velocity 4144/0/0  extent 33152/0/0  colour 0/4144/4144
  GBufferA=ok(1920x1080,fmt24) GBufferB=ok(1920x1080,fmt90) GBufferC=ok(1920x1080,fmt90)
  GBufferD=ok(1920x1080,fmt90) GBufferE=ok(1920x1080,fmt90) SceneDepthZ=ok(1920x1080,fmt19)
...
**Every acceptance criterion the design set for itself passed on hardware.** `nameBad=0` says
the hook is on `FindFreeElement` and not on something that merely looked like it; `faults=0`
across 71 203 calls says the guarded read is safe; `notRegistered=0` says the
`Out -> +8/+16 -> vtable slot 7 GetNativeResource` walk resolves to a resource our own registry
knows, every single time. Depth and velocity **agree 4144 / disagree 0** against L1's
`FPassInputs` — two independent engine routes to one texture, which is the strongest form this
assertion can take — and every scene-buffer-extent name matches View row 132 across 33 152
observations with no disagreement.

**Consequence: RR's guide set is now addressable by name**, at the correct extent, with no
heuristic finder. So are `ScreenSpaceAO` and `SceneColorDeferred`. This is the prerequisite the
RR and bias-mask work was blocked on.

**One observation, not a failure:** `colour` reports agree 0 / disagree 4744 / absent 4744 — the
