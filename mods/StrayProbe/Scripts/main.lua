...
-- A FLAG WITH NO EXPIRY IS A LANDMINE (measured 2026-09-04). A stray-probe-quiet dropped by a
-- session on 2026-09-03 12:52 was still there the next day, so for TWENTY-ONE HOURS the probe
-- wrote seq/t and nothing else - and every launcher run that gates on ingame=1 drove the menu
-- for its full budget against a probe that could not answer, while the game sat in gameplay the
-- whole time. A real measurement window is seconds to a couple of minutes; past the bound the
-- flag is a leak, so ignore it and say so once. ASYNC THREAD ONLY, like flagPresent itself.
local QUIET_MAX_SECONDS = 300
local quietSince, quietExpired = nil, false
local function quietActive()
    if not flagPresent(QUIET_FLAG) then
    ...
    if not quietExpired then
        quietExpired = true
        print(string.format("[StrayProbe] IGNORING %s: held for more than %d s, so it is a "
            .. "leftover rather than a measurement window. Engine queries resume; delete the "
            .. "file to silence this.\n", QUIET_FLAG, QUIET_MAX_SECONDS))
    end
    return false
end
