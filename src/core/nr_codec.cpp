...
// Mirrors shaders/nr_codec.hlsli exactly: the HYBRID knee (identity below kSoftClipKnee, an
// unclipped Neutwo of the excess above), applied as one scalar on the peak channel.
float soft_clip_channel(float v)
{
	if (v <= kSoftClipKnee)
		return v;
	const float headroom = 1.0f - kSoftClipKnee;
	const float e = (v - kSoftClipKnee) / headroom;
	return kSoftClipKnee + headroom * (e / std::sqrt(e * e + 1.0f));
}

Float3 soft_clip(Float3 v)
{
	v = Float3{ std::max(v.x, 0.0f), std::max(v.y, 0.0f), std::max(v.z, 0.0f) };
	const float m = std::max(v.x, std::max(v.y, v.z));
	if (m <= kSoftClipKnee || m <= 1e-6f)
		return v;
	const float k = soft_clip_channel(m) / m;
