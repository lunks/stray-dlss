...
struct Pending
{
	...
	// Texel-logging captures read back four RGBA32F floats and LOG them, rather than
	// writing a file. `file` then holds the log label, not a filename.
	bool log_texel = false;
	std::uint64_t resource_ptr = 0; // the captured resource, for the log line only
};
                   ...
                   std::uint64_t evaluate_count)
{
	...
	if (FAILED(hr))
	{
		STRAY_LOG_ERROR("input_dump: texel readback alloc failed for %s (hr=0x%08x)", label,
			static_cast<unsigned int>(hr));
		return false;
	}
	...
	p.presents_left = 5;
	p.log_texel = true;
	p.resource_ptr = reinterpret_cast<std::uint64_t>(resource);
	std::snprintf(p.file, sizeof(p.file), "%s", label);
	STRAY_LOG_INFO("input_dump: texel capture queued %s at evaluate %llu — %ux%u fmt=%d "
		"(%llu bytes); value logged in ~5 presents.", label,
		static_cast<unsigned long long>(evaluate_count),
		static_cast<unsigned int>(footprint.Footprint.Width), footprint.Footprint.Height,
		static_cast<int>(footprint.Footprint.Format),
		static_cast<unsigned long long>(total_bytes));
...
void on_present()
{
	...
	for (auto it = g_pending.begin(); it != g_pending.end();)
	{
		...
		if (it->log_texel)
		{
			...
			{
				STRAY_LOG_ERROR("input_dump: texel map failed for %s", it->file);
			}
			it->readback->Release();
