...
// queue and no pacer (facts §30.1) — EvaluateFeature records interpolation work onto the
...
// snippet (facts §30.2, tools/ngx_param_names.py). An unknown name is silently ignored
// (CLAUDE.md §5), so a name NOT in that list must never be added without re-running the search.
//
// INPUTS (facts §30.2/30.3): DLSSG.Backbuffer = the game's final frame (the replacement back
